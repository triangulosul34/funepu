...
$sql = 'select count(*), exame_id from pedido_item where ' . ORIGEM_CONFIG . str_pad($atendimento, 7, '0', STR_PAD_LEFT) . " = substring(ordem_servico, 5,9)::integer and situacao in ('Digitado','Liberado') group by 2 order by count desc";
			...
			if ($row = pg_fetch_object($result) == '') {
				...
				while ($row = pg_fetch_object($result)) {
					...
					$sqll = "INSERT INTO pessoas(tipo_pessoa, nome, nome_social, nome_mae, nome_pai, rg, uf_identidade, cpf, data_nascimento, sexo, necessidades_especiais, celular, telefone, email, foto, cep, rua, bairro, cidade, estado, usuario, senha, crbm, pessoa_id_origem, origem) 
						VALUES (3, '" . ts_decodifica($row->nome) . "', '', '" . ts_decodifica($row->nome_mae) . "', '" . ts_decodifica($row->nome_pai) . "', '" . ts_decodifica($row->identidade) . "', '$row->org_expeditor', '" . ts_decodifica($row->cpf) . "', '$row->dt_nasc', $sexo, 1, '$row->celular', '$row->telefone', '', '', '$row->cep', '$row->endereco', '$row->bairro', '$row->cidade', '$row->estado', 'NA', 'NA', '', $paciente_id, " . PORIGEM_CONFIG . ')';
