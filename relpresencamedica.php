...
$stmt1 = "SELECT a.med_atendimento,nome, p.pessoa_id, count(*) as qtd,dat_cad,
		CASE
		WHEN a.hora_destino>='00:00' and a.hora_destino<'07:00' THEN '00h'
		...
		END as entrada,
		CASE
		WHEN a.hora_destino>='00:00' and a.hora_destino<'07:00' THEN '06:59'
...
while ($stmt1 = pg_fetch_object($sth1)) {
	...
	if ($stmt1->nome <> '') {
		...
		$pdf->Cell(70, 8, utf8_decode(substr($stmt1->nome, 0, 35)), 1, 0, 'L');
...
while ($stmt2 = pg_fetch_object($sth2)) {
	if ($stmt2->nome <> '') {
		...
		$pdf->Cell(70, 8, utf8_decode(substr($stmt2->nome, 0, 35)), 1, 0, 'L');
