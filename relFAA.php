...
if ($row->medico_finalizador) {
	$medico_atendimento = ts_decodifica($row->medico_finalizador);
} else {
...
class PDF extends FPDF
{
	...
	{*/
		...
		if ($perfil == '03') {
			$this->Cell(120, 7, utf8_decode('Medica(o): ' . ts_decodifica($usuario_triagem) . ' - CRM: ' . $coren), 0, 0, 'L');
		} else {
			$this->Cell(120, 7, utf8_decode('Enfermeira(o): ' . ts_decodifica($usuario_triagem) . ' - COREN: ' . $coren), 0, 0, 'L');
		}
		...
		if ($destino_paciente == '01') {
			$destino = 'ALTA';
		} elseif ($destino_paciente == '02') {
			$destino = 'ALTA / ENCAM. AMBUL.';
		} elseif ($destino_paciente == '07') {
			$destino = 'EM OBSERVAÇÃO / MEDICAÇÃO';
		} elseif ($destino_paciente == '10') {
			$destino = 'EXAMES / REAVALIACAO';
		} elseif ($destino_paciente == '03') {
			$destino = 'PERMANÊCIA.';
		} elseif ($destino_paciente == '04') {
			$destino = 'TRANSF. OUTRA UPA';
		} elseif ($destino_paciente == '05') {
			$destino = 'TRANSF. INTERN. HOSPITALAR';
		} elseif ($destino_paciente == '06') {
			$destino = 'ÓBITO';
		} elseif ($destino_paciente == '08') {
			$destino = 'NAO RESPONDEU CHAMADO';
		} elseif ($destino_paciente == '09') {
			$destino = 'NAO RESPONDEU CHAMADO';
		} elseif ($destino_paciente == '11') {
			$destino = 'ALTA EVASÃO';
		} elseif ($destino_paciente == '12') {
			$destino = 'ALTA PEDIDO';
		} elseif ($destino_paciente == '14') {
			$destino = 'ALTA / PM';
		} elseif ($destino_paciente == '15') {
