...
function Codabar($xpos, $ypos, $code, $start = 'A', $end = 'A', $basewidth = 10, $height = 0.35)
{
    $barChar = array(
...
$stmt = "
		select a.hora_atendimento, a.dat_atendimento, a.transacao, a.cid_principal,a,hora_cad,l.nome as nomecad, a.destino_paciente, a.data_destino, a.queixa, a.exame_fisico, a.diagnostico_principal,a.prioridade,
		...
		left join classificacao x ON ltrim(x.atendimento_id, '0')= '$pres' 
		where a.transacao=$pres";
...
$dat_atendimento = inverteData($row->dat_atendimento);
...
class PDF extends FPDF
{
    ...
    {
        // Logo
        global $transacao, $pr, $prontuario, $nome, $nome_mae, $sexo, $idade, $nome_convenio, $origem, $enfermaria, $leito, $solicitante, $dt_nasc,
            ...
            $data_destino, $data_transacao, $hora_destino, $destino_paciente, $dor, $oxigenio, $nomecad, $medico_atendimento, $nome_social, $origem_chegada, $prescricao_id, $data, $hora, $sequencias, $dat_atendimento, $hora_atendimento;
        if ($hora_destino == '') {
        ...
        $this->Cell(500, 5, inverteData($data_transacao) . ' - ' . $hora_destino, 0, 0, 'C');
        $this->SetY($voltar);
        ...
        $this->Cell(85, 5, 'UNIDADE DE PRONTO ATENDIMENTO '. utf8_decode(UNIDADE_CONFIG) , 0, 0, 'L');
        $this->SetFont('Arial', '', 9);
        ...
        include('conexao.php');
        $sql = "select * from prescricoes where prescricao_id = $transacao";
        ...
        $this->Cell(20, 5, inverteData($rowd->data), 0, 0, 'L');
        $this->SetFont('Arial', '', 9);
        $this->Cell(12, 5, 'HORA:', 0, 'R');
        $this->SetFont('Arial', '', 9);
        ...
        $this->SetFont('Arial', '', 9);
        $this->Cell(18, 5, 'Atendimento:', 0, 'R');
        $this->SetFont('Arial', '', 9);
        $this->Cell(20, 5, $pr, 0, 0, 'L');
        $this->Ln(5);
        ...
        $this->Cell(145, 5, UTF8_DECODE(' PRESCRIÇÃO '), 1, 0, 'C');
    ...
    function Codabar($xpos, $ypos, $code, $start = 'A', $end = 'A', $basewidth = 0.35, $height = 10)
    {
        ...
        $this->SetFont('Arial', '', 13);
        $this->Text($xpos, 10 + $height + 4, $code);
        $this->SetFillColor(0);
        $code = strtoupper($start . $code . $end);
        for ($i = 0; $i < strlen($code); $i++) {
            $char = $code[$i];
            ...
            for ($bar = 0; $bar < 7; $bar++) {
                $lineWidth = $basewidth * $seq[$bar] / 6.5;
                if ($bar % 2 == 0) {
                    $this->Rect($xpos, 10, $lineWidth, $height, 'F');
                }
                $xpos += $lineWidth;
            }
            $xpos += $basewidth * 10.4 / 6.5;
...
if (isset($_POST['transacao'])) {
    ...
    $query = "select * from prescricoes a
					...
					where a.prescricao_id = $transacao order by tipo";
}
...
$titulomedpaciente = "n";
$medicamentos = array("105280", "105275", "105050", "105266", "104920", "105242", "105195", "105165", "105072", "105013", "105293", "105237", "105310", "105288", "104886", "104885", "105247", "106588", "105245", "105243", "105052", "105051", "105190", "105094", "105455", "105250", "104888", "105194", "104925", "105164", "105240", "107583", "105082", "105081", "105084", "105342", "106568", "106579", "105338", "105206", "105205", "105201");
while ($rows = pg_fetch_object($sthquery)) {
    ...
    if ($rows->tipo == 5) {
        $yy = $pdf->GetX();
        $xx = $pdf->GetY();
        $array_procedimento = explode(",", $rows->codigo_medicamento);
        foreach ($array_procedimento as $value) {
            if (in_array($value, $medicamentos)) {
                $pti = 'true';
            }
        }
        ...
        if ($rows->obs_med != "") {
            $ln = 10;
            $pdf->MultiCell(145, 5, utf8_decode($rows->descricao . ' - ' . $rows->dosagem . ' - ' . $rows->aprazamento . ' - ' . $rows->complemento . "\n      Hipótese Diagnostica: " . $rows->obs_med), 1, 'L', $pti);
        } else {
            $ln = 5;
            $pdf->Cell(145, 5, utf8_decode($rows->descricao . ' - ' . $rows->dosagem . ' - ' . $rows->aprazamento . ' - ' . $rows->complemento), 1, 'R', $pti);
        }

        $yyy = $pdf->GetY();
        $pdf->SetXY($yy + 145, $xx);
        $pdf->Cell(28, $ln, UTF8_DECODE($rows->via), 1, 0, 'C');
        $pdf->Cell(8.5, $ln, UTF8_DECODE('  '), 1, 0, 'C');
        $pdf->Cell(8.5, $ln, UTF8_DECODE('  '), 1, 0, 'C');
        $pdf->Cell(8.5, $ln, UTF8_DECODE('  '), 1, 0, 'C');
        $pdf->Cell(8.5, $ln, UTF8_DECODE('  '), 1, 0, 'C');
        $pdf->Cell(8.5, $ln, UTF8_DECODE('  '), 1, 0, 'C');
    ...
    if ($rows->tipo == 6) {
        ...
        if (($rows->obs_med != "") and ($rows->bomba == '1')) {
            $pdf->MultiCell(145, 5, utf8_decode($descricaof . ' - ' . $rows->aprazamento . ' - ' . $rows->complemento . "\n DILUENTE -> " . $rows->diluente . "\nEm Bomba de Infusao\n      Hipótese Diagnostica: " . $rows->obs_med), 1, 'L', $pti);
            $n = $n + 5;
        } else if ($rows->obs_med != "") {
            $pdf->MultiCell(145, 5, utf8_decode($descricaof . ' - ' . $rows->aprazamento . ' - ' . $rows->complemento . "\n     DILUENTE -> " . $rows->diluente . "\n     Hipótese Diagnostica: " . $rows->obs_med), 1, 'L', $pti);
            $n = $n + 5;
        } else if ($rows->bomba == '1') {
            $pdf->MultiCell(145, 5, utf8_decode($descricaof . ' - ' . $rows->aprazamento . ' - ' . $rows->complemento . "\n       DILUENTE -> " . $rows->diluente . "\n       Em Bomba de Infusao"), 1, 'L', $pti);
        } else {
            $pdf->MultiCell(145, 5, utf8_decode($descricaof . ' - ' . $rows->aprazamento . ' - ' . $rows->complemento . "\n       DILUENTE -> " . $rows->diluente), 1, 'L', $pti);
        }
    ...
    if ($rows->tipo == 1) {
        ...
        //$xx = $pdf->GetY();
        //if($rows->bomba == '1'){
        //$pdf->MultiCell(115,5,utf8_decode($rows->descricao.' - '.$rows->dosagem.' - '.$rows->aprazamento.' - '.$rows->complemento."\nEm Bomba de Infusao"),1,'L');
        //$n = $n + 5;	
        //}else{	
        $pdf->Cell(145, 5, utf8_decode($rows->descricao . ' - ' . $dose . ' - ' . $rows->aprazamento . ' - ' . $rows->complemento), 1, 'R');
        //}
        //$yyy = $pdf->GetY();
        //$pdf->SetXY($yy + 115,$xx);
        $pdf->Cell(28, 5, UTF8_DECODE($rows->via), 1, 0, 'C');
    ...
    if ($rows->tipo == 10) {
        $pdf->Cell(145, 5, utf8_decode($rows->descricao . ' - ' . $rows->dosagem . ' - ' . $rows->aprazamento . ' - ' . $rows->complemento), 1, 'R');
    ...
    if ($rows->tipo == 11) {
        ...
        $pdf->Cell(145, 5, utf8_decode($rows->descricao), 1, 'R');
        $pdf->Cell(28, 5, UTF8_DECODE('  '), 1, 0, 'C');
        $pdf->Cell(8.5, 5, UTF8_DECODE('  '), 1, 0, 'C');
        $pdf->Cell(8.5, 5, UTF8_DECODE('  '), 1, 0, 'C');
        $pdf->Cell(8.5, 5, UTF8_DECODE('  '), 1, 0, 'C');
